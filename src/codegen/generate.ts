...
function renderFlutterNode(
  ...
  if (action) {
    ...
    content = `Semantics(button: true, label: ${quoteDart(node.name)}, child: GestureDetector(onTap: () {}, child: ${content}))`;
  }
...
function generateFlutter(snapshot: DesignSnapshot): GeneratedCode {
  ...
  const source = `// Generated by DesignPort for Flutter 3.47+ core widgets.
// Layout metadata becomes Row, Column, Wrap, Expanded, or Flexible where available.
// This output intentionally stays independent of design-system packages.
...
class DesignPortApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return WidgetsApp(
      ...
      builder: (_, _) => const DesignPortScreen(),
    );
  }
}
...
class DesignPortScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return SafeArea(child: ${body ?? "const SizedBox.shrink()"});
  }
}
`;
  return {
